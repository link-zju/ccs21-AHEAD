...
def ahead_tree_query_error_recorder(ahead_forest, ahead_tree_list, user_dataset, query_interval_table, domain_size, MSEDict, domain_combination_list, data_dimension, branch):
    ...
    for i, query_interval in enumerate(query_interval_table):
        ...
        real_frequency_value = user_dataset[d1_left:d1_right, d2_left:d2_right, d3_left:d3_right].sum()/user_dataset_size

        vector_length = branch[0] ** data_dimension
        ...
        for j, domain_combination in enumerate(domain_combination_list):
            domain_combination_query = np.array([domain_combination[0]*branch[0], domain_combination[0]*branch[0] + 1, domain_combination[1]*branch[1], domain_combination[1]*branch[1] + 1])
...
def theta_calculation(ahead_tree_height, epsilon, user_scale, branch, data_dimension):
    attribute_combination = data_dimension * (data_dimension - 1) / 2
    user_scale_in_each_layer = user_scale / (ahead_tree_height * attribute_combination)
...
if __name__ == "__main__":
    ...
    repeat_time = 1
    epsilon = 1
    domain_size = [2 ** 6, 2 ** 6, 2 ** 6]
    branch = np.array([2, 2, 2])
    data_dimension = 3

    data_name = '3dim_Laplace09'
    data_size_name = 'Set_10_6'
    domain_name = 'Domain_6_6_6'
    ...
    query_path = './querytable/Rand_QueryTable_Domain_6_6_6.npy'
    user_dataset = user_record(data_path, domain_size)
    query_interval_table = np.load(query_path)
    print(query_interval_table.shape)
    SNR = theta_calculation(ahead_tree_height, epsilon, user_dataset.sum(), branch, data_dimension)
